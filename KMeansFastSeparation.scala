...
/**
  ...
  *
  * The original K-means implementation was modified to have two metrics (sse and changed points) which can be
  * used to terminate earlier.
  */
class KMeansFastSeparation private(
                                    private var k: Int,
  ...
  def getSeparationThreshold: Double = SEPARATION_THRESHOLD

  def setSeparationThreshold(threshold: Double): this.type = {
    this.SEPARATION_THRESHOLD = scala.math.abs(threshold)
    this
  }
  ...
  private def runAlgorithm(
    ...
    var separationConverged = false
    var separation = 0.0
    var oldSeparation = Double.NegativeInfinity
    var separationChangedRate = 0.0
    var separationChangedRate_stop = 0.0


    var time_stop = 0L
    var iteration_stop = 0
    var sse_stop = 0.0
    var separation_stop = 0.0
    var old_separation_stop = 0.0
    ...
    while (iteration < maxIterations && !converged) {
      ...
      val sseAccum = sc.doubleAccumulator
      ...
      newCenters.foreach { case (j, newCenter) =>
        val (centerConverged, _) = distanceMeasureInstance.isCenterConverged(centers(j), newCenter, epsilon, true)
        if (converged && !centerConverged) {
          converged = false
        }
        centers(j) = newCenter
      }
      ...
      separation = 0.0
      //calc separation
      centers.foreach{ case(center) =>
        centers.foreach{ case(otherCenter) =>
            separation += distanceMeasureInstance.distance(center, otherCenter)
        }
      }

      separationChangedRate = scala.math.abs(((oldSeparation - separation) / oldSeparation))
      cost = costAccum.value
      iteration += 1
      sse = sseAccum.value

      if (!separationConverged && separationChangedRate <= SEPARATION_THRESHOLD && iteration >1) {
        separationConverged = true
        logInfo("metric convergence reached")
        time_stop = System.nanoTime() - iterationStartTime
        iteration_stop = iteration
        sse_stop = sse
        separation_stop = separation
        old_separation_stop = oldSeparation
        separationChangedRate_stop = separationChangedRate
      }
      oldSeparation = separation

    }

    val iterationTime = System.nanoTime() - iterationStartTime
    ...
    sb.append("Iterations saved: " + (iteration - iteration_stop) + "\n")
    sb.append("Iterations end: " + iteration + "\n")
    sb.append("Time saved: " + (iterationTime - time_stop) / 1e9 + "\n")
    sb.append("Time end: " + iterationTime / 1e9 + "\n")
    sb.append("Quality loss in %: " + ((sse_stop - sse) / sse_stop) * 100 + "\n")
    sb.append("sse stop: " + sse_stop + "\n")
    sb.append("sse end: " + sse + "\n")
    sb.append("separation changed rate at convergence: "+ separationChangedRate_stop +"\n")
    sb.append("separation at convergence: " + separation_stop+ "\n")
    sb.append("old separation at convergence: " + old_separation_stop+ "\n"+ "\n"+ "\n")
    ...
    while (step < initializationSteps) {
      ...
      val chosen = data.zip(costs).mapPartitionsWithIndex {
        (index, pointCosts) =>
          ...
          pointCosts.filter {
            case (_, c) => rand.nextDouble() < 2.0 * c * k / sumCosts
          }.map(_._1)
      }.collect()
...
object KMeansFastSeparation {
  ...
  def train(
    ...
    new KMeansFastSeparation().setK(k)
  ...
  def train(
             ...
             initializationMode: String,
             seed: Long,
             separationThreshold: Double): KMeansModel = {
    new KMeansFastSeparation().setK(k)
      .setMaxIterations(maxIterations)
      .setInitializationMode(initializationMode)
      .setSeed(seed)
      .setSeparationThreshold(separationThreshold)
      .run(data)
  }
